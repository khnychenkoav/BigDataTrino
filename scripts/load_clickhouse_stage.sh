...
client="clickhouse-client --host clickhouse --user lab --password lab --multiquery"

$client < /sql/clickhouse/00_create_all.sql

for file in "MOCK_DATA.csv" "MOCK_DATA (1).csv" "MOCK_DATA (2).csv" "MOCK_DATA (3).csv" "MOCK_DATA (4).csv"; do
    clickhouse-client --host clickhouse --user lab --password lab --query "ALTER TABLE reports.stage_mock_data_raw MODIFY COLUMN source_file String DEFAULT '$file'"
    clickhouse-client --host clickhouse --user lab --password lab --query "INSERT INTO reports.stage_mock_data_raw (
    ...
    ) FORMAT CSVWithNames" < "/data/$file"
done

clickhouse-client --host clickhouse --user lab --password lab --query "ALTER TABLE reports.stage_mock_data_raw MODIFY COLUMN source_file String DEFAULT 'unknown'"
